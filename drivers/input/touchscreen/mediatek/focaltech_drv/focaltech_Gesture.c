...
#define GESTURE_C		    							0x34//zhangjian add
	...
	switch(gesture_id)
	{
          ...
          case GESTURE_LEFT:
           ...
           case GESTURE_RIGHT:
                  ...
                  break;
           case GESTURE_DOUBLECLICK:
                   input_report_key(input_dev, KEY_GESTURE_DOUBLE_TAP, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_DOUBLE_TAP, 0);
                   input_sync(input_dev);
                   break;
           case GESTURE_C:
                   input_report_key(input_dev, KEY_GESTURE_CHAR_C, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_C, 0);
                   input_sync(input_dev);
                   break;			
           case GESTURE_O:
                   input_report_key(input_dev, KEY_GESTURE_CHAR_O, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_O, 0);
                   input_sync(input_dev);
                   break;
           case GESTURE_W:
                   input_report_key(input_dev, KEY_GESTURE_CHAR_W, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_W, 0);
                   input_sync(input_dev);
                   break;
           case GESTURE_M:
                   input_report_key(input_dev, KEY_GESTURE_CHAR_M, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_M, 0);
                   input_sync(input_dev);
                   break;
           case GESTURE_E:
                   input_report_key(input_dev, KEY_GESTURE_CHAR_E, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_E, 0);
                   input_sync(input_dev);
                   break;	       
           case GESTURE_S:
                   input_report_key(input_dev,KEY_GESTURE_CHAR_S, 1);
                   input_sync(input_dev);
                   input_report_key(input_dev, KEY_GESTURE_CHAR_S, 0);
                   input_sync(input_dev);
                   break;
...
int fts_read_Gestruedata(void)
{
    ...
    int gestrue_id = 0;    
    buf  = 0xd3;     
    ret = fts_i2c_read(fts_i2c_client, &buf, 1, &buf, 1);
    printk( "tpd read FTS_GESTRUE buf is 0x%x \n",buf);    
    if (ret < 0)
    {
        printk( "%s read touchdata failed.\n", __func__);
        return ret;
