...
static ssize_t fts_tpd_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    {            
        case 0x15:	   
            len += sprintf(buffer_tpd+len, "TP module: chuangwei (0x15)\n");
            break;
        case 0x67:	   
            len += sprintf(buffer_tpd+len, "TP module: dijing (0x67)\n");
            break;  
	  case 0xB5:	   
            len += sprintf(buffer_tpd+len, "TP module: yashi (0xb5)\n");
            break;  		
        default:
...
int  fts_create_proc(void)	
{
    ...
    mt_entry = proc_create("driver/tsc_id", 0644, NULL, &fts_tpd_id);
    if (mt_entry)
    {
...
{
	int retval = TPD_OK;
	//u8 report_rate = 0;/*ZSW_MODIFY,modify for yashi lcd backlight flashing,yangchaofeng,20161212*/
	int reset_count = 0;
	char data;
	int err = 0;
	...
	/* set INT mode */
       #if 0 //zhangjian delete for Mstar TP
...
reset_proc:
	...
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
		...
		msg_dma_release();
		#endif
               //zhangjina add free DMA buffer
               #ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
               if (tpd_i2c_dma_va)
               {
                   dma_free_coherent(NULL, 250, tpd_i2c_dma_va, tpd_i2c_dma_pa);
                   tpd_i2c_dma_va = NULL;
                   tpd_i2c_dma_pa = 0;
               }
               #endif
...
#endif*/
/*ZSW_MODIFY begin,modify for yashi lcd backlight flashing,yangchaofeng,20161212*/
#if 0
...
#endif
/*ZSW_MODIFY end,modify for yashi lcd backlight flashing,yangchaofeng,20161212*/
	/* tpd_load_status = 1; */
...
#endif
      //zhangjian add release all finger
      input_mt_sync(tpd->dev);
      input_sync(tpd->dev);	
