...
/*ZSW_MODIFY begin,modify for LCD backligth level,yangchaofeng,20161107*/
static unsigned char zte_backlight[256]={
...
/*ZSW_MODIFY end,modify for LCD backligth level,yangchaofeng,20161107*/
...
#endif
static int last_backlevel=-1;//In lk,lcd is on
static void lcm_setbacklight(unsigned int bl)
{
        unsigned char level_high,level_low;

	// Refresh value of backlight level.
	int level;
	level = zte_backlight[bl];

	if(last_backlevel==0 && level !=0)
		lcm_backlight_level_setting[2].para_list[0] = 0x24;//close dimming
	else if(level==0)
		lcm_backlight_level_setting[2].para_list[0] =0x0;
	else
		lcm_backlight_level_setting[2].para_list[0] =0x2c;//open dimming
	...
	lcm_backlight_level_setting[4].para_list[0] = level_high;
	lcm_backlight_level_setting[4].para_list[1] = level_low;
	
#ifdef BUILD_LK
	dprintf(0,"%s,lk backlight:bl=%d -> level = %d\n", __func__, bl,level);
#else
	printk("%s,kernel backlight:bl=%d -> new level=0x%x evel_high,level_low=0x%x,0x%x\n", __func__,bl, level,level_high, level_low);
#endif       
