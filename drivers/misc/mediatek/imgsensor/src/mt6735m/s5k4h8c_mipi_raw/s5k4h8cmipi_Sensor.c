...
#include "s5k4h8cmipi_otp.h"

#define PFX "S5K4H8C_camera_sensor"
...
extern bool otp_update_s5k4h8c(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H8C_SENSOR_ID,
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			LOG_INF("ZLX-chenyunchuanget_s5k4h8C_id() sensor_id: 0x%x\n ", *sensor_id);
			//*sensor_id = imgsensor_info.sensor_id;
		    my_sensor_id=(*sensor_id)+2;
			LOG_INF("ZLX-chenyunchuanget_s5k4h8_id()my_sensor_id : 0x%x\n ", my_sensor_id);
            if (my_sensor_id== imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
               // return ERROR_NONE;
               lens_otp=S5K4H8_OTP_combination_read();	
		if(lens_otp)
		{
			 printk("ZLX-chenyunchuan===s5k4h8--OK--i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,my_sensor_id);
...
static kal_uint32 open(void)
{
	...
	//sensor have two i2c address 0x5a 0x5b & 0x21 0x20, we should detect the module used i2c address
		...
		sensor_id+=2; 	
			//sensor_id = imgsensor_info.sensor_id;
            if (sensor_id == imgsensor_info.sensor_id) {                
                LOG_INF("i2c write id: 0x%x,sensor_id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
                break;
            }   
...
UINT32 S5K4H8C_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
