...
#include "s5k4h8cmipiraw_Sensor.h"
//#include "s5k4h5ymipiraw_Camera_Sensor_para.h"
//#include "s5k4h5ymipiraw_CameraCustomized.h"

//#include "4h5_otp.h"

#define S5K4H8c_DEBUG
#ifdef S5K4H8c_DEBUG
//#define LOG_TAG (__FUNCTION__)
#define PFX "s5k4h8cmipi_otp"
#define LOG_INF(format, args...)	pr_debug("s5k4h8c_otp" "%s: " format, __FUNCTION__ , ##args) 
...
#define s5k4h8cMIPI_WRITE_ID   0x5a
...
#define s5k4h8c_write_cmos_sensor(addr, para)  iWriteReg((u16) addr , (u32) para , 1, s5k4h8cMIPI_WRITE_ID)//add by hhl
...
kal_uint16 s5k4h8c_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	iReadReg((u16) addr ,(u8*)&get_byte,s5k4h8cMIPI_WRITE_ID);
	return get_byte;
}
/*************************************************************************************************
 ...
 * Return      :  void
 **************************************************************************************************/
void start_read_otp_c(void)
{

	s5k4h8c_write_cmos_sensor(0x0100, 0x01);
	s5k4h8c_write_cmos_sensor(0x0A02, 0x0F);   //Select the page to write by writing to 0xD0000A02 0x01~0x0C
	s5k4h8c_write_cmos_sensor(0x0A00, 0x01);   //Enter read mode by writing 01h to 0xD0000A00
	Sleep(20);
}
 ...
 **************************************************************************************************/
void stop_read_otp_c(void)
{
	s5k4h8c_write_cmos_sensor(0x0A00, 0x00);   //Reset the NVM interface by writing 00h to 0xD0000A00
}
 ...
 **************************************************************************************************/
BYTE get_otp_flag_c(BYTE zone)
{
	BYTE flag = 0;
	start_read_otp_c();
	if(zone==1)
	{
		flag = s5k4h8c_read_cmos_sensor(0x0A04);

	}
	if(zone==2)
	{
		flag = s5k4h8c_read_cmos_sensor(0x0A1A);

	}
	LOG_INF("get_otp_flag_c: flag=%d\n", zone);
	stop_read_otp_c();
...
bool get_otp_date_c(BYTE zone) 
{
	...
	BYTE day   = 0;
	start_read_otp_c();
	year  = s5k4h8c_read_cmos_sensor(0x0A06+(zone*WB_OFFSET));
	month = s5k4h8c_read_cmos_sensor(0x0A07+(zone*WB_OFFSET));
	day   = s5k4h8c_read_cmos_sensor(0x0A08+(zone*WB_OFFSET));
	stop_read_otp_c();
 ...
 **************************************************************************************************/
#if 0
BYTE get_otp_module_id_c(BYTE zone)
{

	BYTE module_id = 0;
	start_read_otp_c();

	module_id  = s5k4h8c_read_cmos_sensor(0x0A05+(zone*WB_OFFSET));

	stop_read_otp_c();

	LOG_INF("OTP_Module ID_C: 0x%02x.\n",module_id);
 ...
 **************************************************************************************************/
bool otp_lenc_update_c(USHORT zone)
{
	LOG_INF("otp_lenc_update_c: flag=%d\n",zone);
	s5k4h8c_write_cmos_sensor(0x0B00, 0x0180);
	s5k4h8c_write_cmos_sensor(0x6028, 0x2000);
	s5k4h8c_write_cmos_sensor(0x602A, 0x0c40);
	s5k4h8c_write_cmos_sensor(0x6F12, 0x0140);
	Sleep(10);

	s5k4h8c_write_cmos_sensor(0x0100, 0x0100);
...
bool wb_gain_set_c(void)
{
	...
	LOG_INF("QYC_OTP_current_rg_c=%d,current_bg=%d\n",Current_RG_C,Current_BG_C);
	LOG_INF("QYC_OTP_r_ratio=%d,b_ratio=%d \n",r_ratio,b_ratio);
#if 1
	s5k4h8c_write_cmos_sensor(0x6028, 0x4000);
	s5k4h8c_write_cmos_sensor(0x602A, 0x3058);
	s5k4h8c_write_cmos_sensor(0x6F12, 0x01);
		
	s5k4h8c_write_cmos_sensor(0x020e, G_GainH);
	s5k4h8c_write_cmos_sensor(0x020f, G_GainL);
	s5k4h8c_write_cmos_sensor(0x0210, R_GainH);
	s5k4h8c_write_cmos_sensor(0x0211, R_GainL);
	s5k4h8c_write_cmos_sensor(0x0212, B_GainH);
	s5k4h8c_write_cmos_sensor(0x0213, B_GainL);
	s5k4h8c_write_cmos_sensor(0x0214, G_GainH);
	s5k4h8c_write_cmos_sensor(0x0215, G_GainL);
#endif
	LOG_INF("QYC_OTP WB_C Update Finished!\n");
	return 1;
...
bool get_otp_wb_c(int zone)
{
	...
	{
		val = s5k4h8c_read_cmos_sensor(i);
		LOG_INF("s5k4h8 get_otp_wb_c: 0x%x=0x%x\n", i, val);
	}
	#endif
	if(zone == 1){
		templ = s5k4h8c_read_cmos_sensor(0x0A0C);// + (zone*WB_OFFSET));
		temph= s5k4h8c_read_cmos_sensor(0x0A0D);// + (zone*WB_OFFSET));

		Current_RG_C = (USHORT)((temph<<8)| templ);

		templ = s5k4h8c_read_cmos_sensor(0x0A0E);//+ (zone*WB_OFFSET));
		temph= s5k4h8c_read_cmos_sensor(0x0A0F);//+ (zone*WB_OFFSET));

		Current_BG_C = (USHORT)((temph<<8)| templ);
	}
	if(zone == 2){
		templ = s5k4h8c_read_cmos_sensor(0x0A22);// + (zone*WB_OFFSET));
		temph = s5k4h8c_read_cmos_sensor(0x0A23);// + (zone*WB_OFFSET));
	
		Current_RG_C = (USHORT)((temph<<8)| templ);
	
		templ = s5k4h8c_read_cmos_sensor(0x0A24);//+ (zone*WB_OFFSET));
		temph = s5k4h8c_read_cmos_sensor(0x0A25);//+ (zone*WB_OFFSET));
	
		Current_BG_C = (USHORT)((temph<<8)| templ);
	}


	LOG_INF("Current_RG_C=0x%x, Current_BG_C=0x%x\n", Current_RG_C, Current_BG_C);
 ...
 **************************************************************************************************/
bool otp_wb_update_c(BYTE zone)
{
	//USHORT golden_g, current_g;


	if(!get_otp_wb_c(zone))  // get wb data from otp
		return 0;

	r_ratio = 512 * Golden_RG / Current_RG_C;
	b_ratio = 512 * Golden_BG / Current_BG_C;

	wb_gain_set_c();

	LOG_INF("otp_wb_update_c finished! \n");
...
1 : update success
 **************************************************************************************************/
bool otp_update_s5k4h8c(void)
{
	BYTE zone = 0x00;
	BYTE FLG = 0x00;
	int i;
	LOG_INF("otp_update_s5k4h8c begain! \n");

	for(i=0;i<3;i++)
	{
		FLG = get_otp_flag_c(zone);
		if(FLG == VALID_OTP)
			break;
		else
			zone++;
	...
	//{
		//	return 0;
	//}
	otp_wb_update_c(zone);	
	otp_lenc_update_c(zone);
	
	LOG_INF("otp_update_s5k4h8c end! \n");
