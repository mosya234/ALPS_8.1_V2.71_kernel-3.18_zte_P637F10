...
#define s5k4h9CMIPI_WRITE_ID   0x20
...
#define s5k4h9c_write_cmos_sensor(addr, para)  iWriteReg((u16) addr , (u32) para , 1, s5k4h9CMIPI_WRITE_ID)//add by hhl
...
#define GAIN_DEFAULT       0x100

#define Golden_RG_4h9   0x269 //0x206  //0x209  // modify by gionee wangxiaoyu 2015-09-01
#define Golden_BG_4h9   0x270 //0x24B  //0x259  // modify by gionee wangxiaoyu 2015-09-01

USHORT Current_RG_4h9c;
USHORT Current_BG_4h9c;


static kal_uint32 r_ratio_4h9c;
static kal_uint32 b_ratio_4h9c;

kal_uint16 s5k4h9c_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	iReadReg((u16) addr ,(u8*)&get_byte,s5k4h9CMIPI_WRITE_ID);
	return get_byte;
}
/*************************************************************************************************
 ...
 * Return      :  void
 **************************************************************************************************/
void start_read_otp_4h9c(void)
{

	s5k4h9c_write_cmos_sensor(0x0100, 0x01);
	s5k4h9c_write_cmos_sensor(0x0A02, 0x0F);   //Select the page to write by writing to 0xD0000A02 0x01~0x0C
	s5k4h9c_write_cmos_sensor(0x0A00, 0x01);   //Enter read mode by writing 01h to 0xD0000A00
	Sleep(20);
}
 ...
 **************************************************************************************************/
void stop_read_otp_4h9c(void)
{
	s5k4h9c_write_cmos_sensor(0x0A00, 0x00);   //Reset the NVM interface by writing 00h to 0xD0000A00
}
 ...
 **************************************************************************************************/
BYTE get_otp_flag_4h9c(BYTE zone)
{
	BYTE flag = 0;
	start_read_otp_4h9c();
	if(zone==1)
	{
		flag = s5k4h9c_read_cmos_sensor(0x0A04);

	}
	if(zone==2)
	{
		flag = s5k4h9c_read_cmos_sensor(0x0A1A);
 ...
 **************************************************************************************************/
bool otp_lenc_update_4h9c(USHORT zone)
{
	LOG_INF("otp_lenc_update_4h9c: flag=%d\n",zone);
	s5k4h9c_write_cmos_sensor(0x0B00, 0x0180);
	s5k4h9c_write_cmos_sensor(0x6028, 0x2000);
	s5k4h9c_write_cmos_sensor(0x602A, 0x0c40);
	s5k4h9c_write_cmos_sensor(0x6F12, 0x0140);
	Sleep(100);

	s5k4h9c_write_cmos_sensor(0x0100, 0x0100);
...
bool wb_gain_set_4h9c(void)
{
	...
	if(r_ratio_4h9c >= 512 )
	{
		if(b_ratio_4h9c>=512) 
		{
			R_GAIN = (USHORT)(GAIN_DEFAULT * r_ratio_4h9c / 512);						
			G_GAIN = GAIN_DEFAULT;
			B_GAIN = (USHORT)(GAIN_DEFAULT * b_ratio_4h9c/ 512);
		}

		else
		{
			R_GAIN =  (USHORT)(GAIN_DEFAULT*r_ratio_4h9c / b_ratio_4h9c );
			G_GAIN = (USHORT)(GAIN_DEFAULT*512 / b_ratio_4h9c );
			B_GAIN = GAIN_DEFAULT;    
		}
	}
	else
	{
		...
		{
			R_GAIN = GAIN_DEFAULT;
			G_GAIN = (USHORT)(GAIN_DEFAULT*512 /r_ratio_4h9c);		
			B_GAIN =  (USHORT)(GAIN_DEFAULT*b_ratio_4h9c / r_ratio_4h9c );
		} 
		else
		{

			Gr_GAIN = (USHORT)(GAIN_DEFAULT*512/ r_ratio_4h9c );						
			Gb_GAIN = (USHORT)(GAIN_DEFAULT*512/b_ratio_4h9c );						
			if(Gr_GAIN >= Gb_GAIN)						
			{						
				R_GAIN = GAIN_DEFAULT;						
				G_GAIN = (USHORT)(GAIN_DEFAULT *512/ r_ratio_4h9c );						
	...
	LOG_INF("QYC_OTP_current_rg=%d,current_bg=%d\n",Current_RG_4h9c,Current_BG_4h9c);
	LOG_INF("QYC_OTP_r_ratio=%d,b_ratio=%d \n",r_ratio_4h9c,b_ratio_4h9c);
#if 1
	s5k4h9c_write_cmos_sensor(0x6028, 0x4000);
	s5k4h9c_write_cmos_sensor(0x602A, 0x3058);
	s5k4h9c_write_cmos_sensor(0x6F12, 0x01);
		
	s5k4h9c_write_cmos_sensor(0x020e, G_GainH);
	s5k4h9c_write_cmos_sensor(0x020f, G_GainL);
	s5k4h9c_write_cmos_sensor(0x0210, R_GainH);
	s5k4h9c_write_cmos_sensor(0x0211, R_GainL);
	s5k4h9c_write_cmos_sensor(0x0212, B_GainH);
	s5k4h9c_write_cmos_sensor(0x0213, B_GainL);
	s5k4h9c_write_cmos_sensor(0x0214, G_GainH);
	s5k4h9c_write_cmos_sensor(0x0215, G_GainL);
#endif
...
bool get_otp_wb_4h9c(int zone)
{
	...
	start_read_otp_4h9c();
	if(zone == 1){
		templ = s5k4h9c_read_cmos_sensor(0x0A0C);// + (zone*WB_OFFSET));
		temph= s5k4h9c_read_cmos_sensor(0x0A0D);// + (zone*WB_OFFSET));

		Current_RG_4h9c = (USHORT)((temph<<8)| templ);

		templ = s5k4h9c_read_cmos_sensor(0x0A0E);//+ (zone*WB_OFFSET));
		temph= s5k4h9c_read_cmos_sensor(0x0A0F);//+ (zone*WB_OFFSET));

		Current_BG_4h9c = (USHORT)((temph<<8)| templ);
	}
	if(zone == 2){
		templ = s5k4h9c_read_cmos_sensor(0x0A22);// + (zone*WB_OFFSET));
		temph = s5k4h9c_read_cmos_sensor(0x0A23);// + (zone*WB_OFFSET));
	
		Current_RG_4h9c = (USHORT)((temph<<8)| templ);
	
		templ = s5k4h9c_read_cmos_sensor(0x0A24);//+ (zone*WB_OFFSET));
		temph = s5k4h9c_read_cmos_sensor(0x0A25);//+ (zone*WB_OFFSET));
 ...
 **************************************************************************************************/
bool otp_wb_update_4h9c(BYTE zone)
{
	//USHORT golden_g, current_g;


	if(!get_otp_wb_4h9c(zone))  // get wb data from otp
		return 0;

	r_ratio_4h9c = 512 * Golden_RG_4h9 / Current_RG_4h9c;
	b_ratio_4h9c = 512 * Golden_BG_4h9 / Current_BG_4h9c;
...
1 : update success
 **************************************************************************************************/
bool otp_update_s5k4h9c(void)
{
	BYTE zone = 0x00;
	BYTE FLG = 0x00;
	int i;
	LOG_INF("otp_update_s5k4h9c begain! \n");

	for(i=0;i<3;i++)
	{
		FLG = get_otp_flag_4h9c(zone);
		if(FLG == VALID_OTP)
			break;
		else
			zone++;
	...
	//{
		//	return 0;
	//}
	otp_wb_update_4h9c(zone);	
	otp_lenc_update_4h9c(zone);
