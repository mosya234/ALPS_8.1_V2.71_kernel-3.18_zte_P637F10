...
#include "s5k4h9cmipiraw_Sensor.h"
#include "s5k4h9cmipi_otp.h"

#define PFX "S5K4H9C_camera_sensor"
...
extern bool otp_wb_update_4h9c(BYTE zone);
extern bool otp_lenc_update_4h9c(USHORT zone);

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H9C_SENSOR_ID,
...
/*************************************************************************
* FUNCTION*/
static UINT32 S5K4H9_OTP_combination_read(void)
{
	int lens_id_1=0;
	int lens_id_2=0;
	...
	{
	printk(" chenyunchuan===s5k4h8 is new lens! \n");
	return TRUE;
	}
	else
	{
				...
				*sensor_id = return_sensor_id();
				...
				{               
                	LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
               		lens_otp=S5K4H9_OTP_combination_read();	
					if(lens_otp)
					{
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	otp_lenc_update_4h9c(1);
	if(!otp_wb_update_4h9c(1))
	{
	  otp_wb_update_4h9c(2);
	}
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K4H9C_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
