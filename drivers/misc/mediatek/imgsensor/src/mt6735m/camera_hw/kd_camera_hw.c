...
#define VOL_1000 1000000
	...
	if (pinSetIdx == 0)
			...
			ISP_MCLK1_EN(1);
	if (currSensorName 
	    && ((0 == strcmp(SENSOR_DRVNAME_S5K4H8_MIPI_RAW, currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K4H8C_MIPI_RAW, currSensorName))))
		{
			/* First Power Pin low and Reset Pin Low */
				...
				//printk("[CAMERA SENSOR] s5k4h8 power on start +++gyy+++ \n");

				//gpio_direction_output(GPIO_CAMERA_LDO_EN_PIN, 1);
	                     //gpio_set_value(GPIO_CAMERA_LDO_EN_PIN, 1);
                mtkcam_gpio_set(0, CAMLDO, 1);//LDO         
			    mdelay(1);
	    ...
	    && ((0 == strcmp(SENSOR_DRVNAME_S5K4H9_MIPI_RAW, currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K4H9C_MIPI_RAW, currSensorName))))
		{
			/* First Power Pin low and Reset Pin Low */
				if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMPDN])
					mtkcam_gpio_set(1, CAMPDN, pinSet[1][IDX_PS_CMPDN + IDX_PS_OFF]);
				...
				//printk("[CAMERA SENSOR] s5k4h9 power on start +++gyy+++ \n");
				if (TRUE != _hwPowerOnCnt(VCAMA, VOL_2800 , mode_name)) 
					{
						PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_A), power id = %d\n", VCAMA);
						goto _kdCISModulePowerOn_exit_;
                ...
                else if ((currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_GC2375_MIPI_RAW))))
                 {
			/* First Power Pin low and Reset Pin Low */
				if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMPDN])
					mtkcam_gpio_set(1, CAMPDN, pinSet[1][IDX_PS_CMPDN + 1]);
				if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMRST])
					mtkcam_gpio_set(1, CAMRST, pinSet[1][IDX_PS_CMRST + IDX_PS_OFF]);
				
				//printk("[CAMERA SENSOR] gc2357 power on start +++gyy+++ \n");
				...
				if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMPDN])
					mtkcam_gpio_set(1, CAMPDN, pinSet[1][IDX_PS_CMPDN + 0]);
				if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMRST])
					mtkcam_gpio_set(1, CAMRST, pinSet[1][IDX_PS_CMRST + IDX_PS_ON]);
	   ...
	   && ((0 == strcmp(SENSOR_DRVNAME_S5K4H8_MIPI_RAW, currSensorName))||(0 == strcmp(SENSOR_DRVNAME_S5K4H8C_MIPI_RAW, currSensorName))))
		{
			...
			/* VCAM_A */
			mtkcam_gpio_set(0, CAMLDO, 0);
			//gpio_direction_output(GPIO_CAMERA_LDO_EN_PIN, 1);
			//gpio_set_value(GPIO_CAMERA_LDO_EN_PIN, 0);
			/* AF_VCC */
			    /* VCAM_AVDD*/
